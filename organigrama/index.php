...
if ( isset( $_POST[ 'nume_filiala' ] ) ) {
	if($_POST['id']){
        if(count($_FILES)){
            $uploads_dir =  THF_ROOT . UPLOAD . 'filiale_management/';
            $file_name = $_POST['id'].'.jpg';
            $r = move_uploaded_file($_FILES['file_logo']['tmp_name'],$uploads_dir.$file_name);
        }
		update_qaf('filiale',$_POST,"id='".floor($_POST['id'])."'",$limit='LIMIT 1', $keys_to_exclude=array('id'), $setify_only_keys=array(), $return_query=false);
	}
	else{
		$last_id = insert_qa( 'filiale', $_POST );
	}
	redirect();
}
...
if ( isset( $search_words ) ) {
	foreach ( $search_words as $word ) {
		$search_sql .= ( $search_sql ? " AND " : "" ) . ( "nume_filiala LIKE '%" . q( $word ) . "%'   " );
	}
}
...
$rows = multiple_query( "SELECT * from filiale $where_sql  ORDER BY nume_filiala ASC" );
...
if ( isset( $_POST[ 'delete_filiala' ] ) ) {
	delete_query( 'delete from filiale where id="' . $_POST[ 'filiala_id' ] . '"' );
			...
			<div class='col-sm-4'>
				<form action="" method="post" enctype="application/x-www-form-urlencoded" class="form-inline">
				  <div class="form-group" style="margin-top: 25px;">
					<input type="text" class='form-control' name='nume_filiala' placeholder="nume filiala">
