...
$pid = floor($_GET['view']);
$cumparatori = $cumparatori_all[ $pid ];
...
//////uploader si salvare generala in tabel vanzare:
    ...
    ThfGalleryEditor::$web_path = UPLOAD . 'cumparatori/';
    ThfGalleryEditor::$sv_path = THF_ROOT . UPLOAD . 'cumparatori/';
...
////3333333
    ...
    if (count($_POST) || $_SERVER['REQUEST_METHOD'] == 'POST') { //prea($_FILES);	prea($_POST); die;
        ...
        ThfGalleryEditor::$database_pics = (is_numeric($pid) && $pid > 0 ? json_decode($cumparatori['atasamente'], true) : array()); //load existing stored pictures
        ...
        if (is_numeric($pid)) { //update
            ...
            update_qaf($tablename, $_POST, "`idc`='$pid'", 'LIMIT 1', $keys_to_exclude = array('idc'), $setify_only_keys = array(), $return_query = false);
...
if($_GET['type'] == 'doc') {
    ...
    ThfGalleryEditor::$accept = 'jpg|jpeg|png|pdf|txt|doc|docx|xls|xlsx';
    ThfGalleryEditor::$web_path = UPLOAD . 'cumparatori_documente/';
    ThfGalleryEditor::$sv_path = THF_ROOT . UPLOAD . 'cumparatori_documente/';
    ThfGalleryEditor::$file_name_policy = '*f*';
    ThfGalleryEditor::$overwritePolicy = 'r';
    ThfGalleryEditor::$id = floor($pid); //specifica ce id are produsul pentru a putea urca asincron poze si file
////3333333
    ...
    if (count($_POST) || $_SERVER['REQUEST_METHOD'] == 'POST') { //prea($_FILES);	prea($_POST); die;
        ...
        ThfGalleryEditor::$database_pics = (is_numeric($pid) && $pid > 0 ? json_decode($cumparatori['atasamente_documente'], true) : array()); //load existing stored pictures
        ThfGalleryEditor::actionFileController(); //unlinks files fron the drive and updates ThfGalleryEditor::$database_pics
        ThfGalleryEditor::uploadController();
        //    ThfGalleryEditor::resize_uploaded_pics($resize_policy,$remove_original_pic=false);
