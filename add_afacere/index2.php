    ...
    <?php if($login_user == 2) {
        ...
        <div class="ui card grid raised">
            <div class="column">
                ...
                <ul class="list-group">
                <?php foreach ($promovari_db as $idp=>$promo){
                    $total +=  isset($repere[$idp]['cantitatea']) ? $repere[$idp]['cantitatea']*$repere[$idp]['pret_unitar'] : 0;
                    $variante = array();?>
                    <li class="list-group-item">
                        <p><?=$promo['nume_promovare']?></p>

                        <?php
                        $varianteX = explode(',',$promo['variante']);
                        foreach ($varianteX as $k){ $variante[$k] = $unitati_masura_promovari[$k];}
                        select_rolem( 'select_'.$idp, '', $variante,$repere[$idp]['cantitatea'],'' );
                       // prea($promo);
                        ?>
                    </li>
                <?php } ?>
                    ...
                    <?php if(is_numeric($promovare_data['idf']) and $promovare_data['status'] > 0){?>
...
<script>
    ...
    function save_all_forms_vanzare() {
    	console.log("xxx"+validate_fields());
    	if(validate_fields())
    {
    ...
    function save_form(type) {
        ...
        $.post("",$(type).serialize(),function (r) {
        ...
        $('#status').change(function () {
           ...
           var msg = '';
            ...
            if(words.length < 120 && $('#status').val() > 0) {
                ...
                alert("Inainte de publicare afacerea trebuie sa aiba minim 7 poze cu o rezolutie de minim 640x480 pixeli!");
                return 0;
            }
